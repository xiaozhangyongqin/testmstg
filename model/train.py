...
def prepare_x_y(x, y):
    ...
    return x0.to(device), y0.to(device), x1.to(device), y1.to(device)  # x, y, x_cov, y_cov
...
def evaluate(model, mode):
    with torch.no_grad():
        ...
        for x, y in data_iter:
            x, y, x_cov, ycov = prepare_x_y(x, y)
            output, h_att, query, pos, neg1, neg2 = model(x, x_cov, ycov)
            y_pred = scaler.inverse_transform(output)
            y_true = scaler.inverse_transform(y)
            loss1 = masked_mae_loss(y_pred, y_true)  # masked_mae_loss(y_pred, y_true)
            loss2 = quadruplet_loss(query, pos, neg1, neg2)
...
def traintest_model():
    ...
    print_model(model)
    # optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, eps=args.epsilon, weight_decay=0)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, eps=args.epsilon, weight_decay=0.01)
    # optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, eps=args.epsilon)
    ...
    for epoch_num in range(args.epochs):
        ...
        for x, y in data_iter:
            optimizer.zero_grad()
            x, y, x_cov, ycov = prepare_x_y(x, y)
            # x.shape: 64, 12, node, 1 # speed
            # y.shape: 64, 12, node, 1 # speed
            # ycov.shape: 64, 12, node, 1 # 时间
            output, h_att, query, pos, neg1, neg2 = model(x, x_cov, ycov, y, batches_seen)
            y_pred = scaler.inverse_transform(output)
            y_true = scaler.inverse_transform(y)

            loss1 = masked_mae_loss(y_pred, y_true)  # masked_mae_loss(y_pred, y_true)

            loss2 = quadruplet_loss(query, pos.detach(), neg1.detach(), neg2.detach())
...
parser.add_argument('--rnn_units', type=int, default=40, help='number of rnn units')
parser.add_argument('--mem_num', type=int, default=40, help='number of meta-nodes/prototypes')
parser.add_argument('--mem_dim', type=int, default=40, help='dimension of meta-nodes/prototypes')
